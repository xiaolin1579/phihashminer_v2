/*
    This file is part of phihashminer.

    phihashminer is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    phihashminer is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with phihashminer.  If not, see <http://www.gnu.org/licenses/>.
*/

#include <CLI/CLI.hpp>

#include <phihashminer/buildinfo.h>
...
bool g_exitOnError = false;  // Whether or not phihashminer should exit on mining threads errors
...
#if ETH_DBUS
#include <phihashminer/DBusInt.h>
#endif
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("phihashminer - GPU PHIHASH(0.9.3) miner for Zing");
    ...
    void help()
    {
        cout << "phihashminer - GPU PHIHASH(0.9.3) miner for Zing" << endl
             << "minimal usage : phihashminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default phihashminer will try to use all devices types" << endl
             ...
             << "                        phihashminer --help-ext con" << endl
             << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    phihashminer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes phihashminer can stay" << endl
                 ...
                 << "    --exit              FLAG Stop phihashminer whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how phihashminer chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects phihashminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    phihashminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'phihashminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When phihashminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode phihashminer tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = phihashminer_get_buildinfo();
    cout << endl
         << endl
         << "phihashminer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'phihashminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try phihashminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
