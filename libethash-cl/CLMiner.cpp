...
void CLMiner::workLoop()
{
    ...
    try
    {
        ...
        while (!shouldStop())
        {
            ...
            {
                uint64_t period_seed = next.block / PHIHASH_PERIOD;
                if (m_nextPhihashPeriod == 0)
                {
                    m_nextPhihashPeriod = period_seed;
                    // g_io_service.post(
                    //    m_phihash_io_strand.wrap(boost::bind(&CLMiner::asyncCompile, this)));
                ...
                if (old_period_seed != period_seed)
                {
                    ...
                    cllog << "Loaded period " << period_seed << " phihash kernel";
                    // g_io_service.post(
                    //    m_phihash_io_strand.wrap(boost::bind(&CLMiner::asyncCompile, this)));
                    m_compileThread = new boost::thread(boost::bind(&CLMiner::asyncCompile, this));
                    continue;
    ...
    try
    {
        ...
        if (!m_settings.noBinary)
        {
            ...
            fname_strm << boost::dll::program_location().parent_path().string()
                       << "/kernels/phihash_" << device_name << "_lws" << m_settings.localWorkSize
                       << ".bin";
...
void CLMiner::asyncCompile()
{
    ...
    compileKernel(m_nextPhihashPeriod, m_nextProgram, m_nextSearchKernel);
...
void CLMiner::compileKernel(uint64_t period_seed, cl::Program& program, cl::Kernel& searchKernel)
{
    ...
    cllog << "Pre-compiled period " << period_seed << " OpenCL PhiHash kernel";
}
