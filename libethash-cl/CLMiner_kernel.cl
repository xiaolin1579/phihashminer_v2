...
#define HASHES_PER_GROUP (GROUP_SIZE / PHIHASH_LANES)
...
typedef struct
{
    ...
    uint64_t uint64s[PHIHASH_LANES / 2];
} shuffle_t;
    ...
    __global dag_t const* g_dag, ulong start_nonce, ulong target, uint hack_false)
{
    ...
    __local uint32_t c_dag[PHIHASH_CACHE_WORDS];

    uint32_t const lid = get_local_id(0);
    uint32_t const gid = get_global_id(0);
    uint64_t const nonce = start_nonce + gid;

    const uint32_t lane_id = lid & (PHIHASH_LANES - 1);
    const uint32_t group_id = lid / PHIHASH_LANES;

    // Load the first portion of the DAG into the cache
    for (uint32_t word = lid * PHIHASH_DAG_LOADS; word < PHIHASH_CACHE_WORDS;
         word += GROUP_SIZE * PHIHASH_DAG_LOADS)
    {
...
#pragma unroll 1
        for (uint32_t l = 0; l < PHIHASH_CNT_DAG; l++)
            PhihashLoop(l, mix, g_dag, c_dag, share[0].uint64s, hack_false);
    ...
    uint start, __global hash64_t const* g_light, __global hash64_t* g_dag, uint isolate)
{
    ...
    if (node_index * sizeof(hash64_t) >= PHIHASH_DAG_BYTES)
        return;
