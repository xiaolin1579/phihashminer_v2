...
/*
   ...
   This happens if a
     * new work arrived                       or
     * miner should stop (eg exit phihashminer)   or
     * miner should pause
*/
...
void CPUMiner::search(const dev::eth::WorkPackage& w)
{
    ...
    const auto& context = phihash::get_global_epoch_context_full(w.epoch);
    auto header = phihash::hash256_from_bytes(w.header.data());
    auto boundary = phihash::hash256_from_bytes(w.get_boundary().data());
    ...
    while (true)
    {
        ...
        auto r = phihash::search(context, w.block, header, boundary, nonce, blocksize);
