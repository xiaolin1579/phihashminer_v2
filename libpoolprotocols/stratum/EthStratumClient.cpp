...
void EthStratumClient::connect_handler(const boost::system::error_code& ec)
{
        ...
        if (hec)
        {
            ...
            {  // certificate verification failed
                ...
                cwarn << "* Disable certificate verification all-together via environment "
                         "variable. See phihashminer --help for info about environment variables";
    ...
    */
    ...
    case EthStratumClient::STRATUM:
        ...
        jReq["jsonrpc"] = "2.0";
        jReq["params"].append(std::string(phihashminer_get_buildinfo()->project_name) + "/" + std::string(phihashminer_get_buildinfo()->project_version));
    ...
    case EthStratumClient::ETHEREUMSTRATUM:
        ...
        jReq["params"].append(phihashminer_get_buildinfo()->project_name_with_version);
    ...
    case EthStratumClient::ETHEREUMSTRATUM2:
        ...
        Json::Value jPrm;
        jPrm["agent"] = phihashminer_get_buildinfo()->project_name_with_version;
...
void EthStratumClient::processResponse(Json::Value& responseObject)
{
        ...
        (_isNotification && (responseObject["params"].empty() && responseObject["result"].empty())))
    {
        ...
        cwarn << "Do not blame phihashminer for this. Ask pool devs to honor http://www.jsonrpc.org/ "
                 "specifications ";
        cwarn << "Disconnecting...";
        m_io_service.post(m_io_strand.wrap(boost::bind(&EthStratumClient::disconnect, this)));
        return;
    ...
    */
    ...
    if (_isNotification && m_conn->StratumModeConfirmed())
    {
        ...
        {
            jReq["id"] = _id;
            jReq["result"] = phihashminer_get_buildinfo()->project_name_with_version;
...
{
    static std::chrono::steady_clock::time_point last_submission_time = std::chrono::steady_clock::now();
    
    // Throttle submissions to a specific rate (e.g., 1 submission every 2 seconds)
    auto current_time = std::chrono::steady_clock::now();
    std::chrono::seconds min_interval(0); // Set your desired interval

    if (std::chrono::duration_cast<std::chrono::seconds>(current_time - last_submission_time) < min_interval)
    {
        // If submission happens too fast, return without submitting.
        cwarn << "Submission rate exceeded. Skipping submission to throttle miner.";
        return;
    }

    // Update the time of the last successful submission
