// Inner loop for phi_seed 600
__device__ __forceinline__ void PhihashLoop(const uint32_t loop,
    uint32_t mix[PHIHASH_REGS],
    const dag_t *g_dag,
    const uint32_t c_dag[PHIHASH_CACHE_WORDS],
    const bool hack_false)
{
    dag_t data_dag;
    uint32_t offset, data;
    const uint32_t lane_id = threadIdx.x & (PHIHASH_LANES - 1);
    // global load
    offset = __shfl_sync(0xFFFFFFFF, mix[0], loop%PHIHASH_LANES, PHIHASH_LANES);
    offset %= PHIHASH_DAG_ELEMENTS;
    offset = offset * PHIHASH_LANES + (lane_id ^ loop) % PHIHASH_LANES;
    data_dag = g_dag[offset];
    // hack to prevent compiler from reordering LD and usage
    if (hack_false) __threadfence_block();
    // cache load 0
    offset = mix[26] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    mix[0] = (mix[0] ^ data) * 33;
    // random math 0
    data = mix[10] ^ mix[16];
    ...
    // cache load 1
    offset = mix[30] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 2
    offset = mix[1] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 3
    offset = mix[19] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    mix[7] = (mix[7] ^ data) * 33;
    // cache load 4
    offset = mix[11] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    mix[8] = (mix[8] * 33) + data;
    // cache load 5
    offset = mix[21] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    mix[9] = (mix[9] ^ data) * 33;
    // random math 5
    ...
    // cache load 6
    offset = mix[15] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 7
    offset = mix[18] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 8
    offset = mix[3] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    mix[6] = (mix[6] ^ data) * 33;
    // random math 8
    ...
    // cache load 9
    offset = mix[17] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    ...
    // cache load 10
    offset = mix[31] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
    mix[2] = (mix[2] * 33) + data;
    // random math 10
    data = mix[11] | mix[17];
    ...
    // cache load 11
    offset = mix[16] % PHIHASH_CACHE_WORDS;
    data = c_dag[offset];
