...
void CUDAMiner::workLoop()
{
    ...
    try
    {
        while (!shouldStop())
        {
            ...
            uint64_t period_seed = w.block / PHIHASH_PERIOD;
            if (m_nextPhihashPeriod == 0)
            {
                m_nextPhihashPeriod = period_seed;
            ...
            if (old_period_seed != period_seed)
            {
                ...
                if (period_seed != m_nextPhihashPeriod)
                {
                ...
                m_kernelExecIx ^= 1;
                cudalog << "Launching period " << period_seed << " PhiHash kernel";
                m_nextPhihashPeriod = period_seed + 1;
                m_compileThread = new boost::thread(boost::bind(&CUDAMiner::asyncCompile, this));
            }
            // Epoch change ?
...
void CUDAMiner::asyncCompile()
{
    ...
    compileKernel(m_nextPhihashPeriod, m_epochContext.dagNumItems / 2, m_kernel[m_kernelCompIx]);
...
void CUDAMiner::compileKernel(uint64_t period_seed, uint64_t dag_elms, CUfunction& kernel)
{
    ...
    std::string op_dag = "-DPHIHASH_DAG_ELEMENTS=" + to_string(dag_elms);
    ...
    cudalog << "Pre-compiled period " << period_seed << " CUDA PhiHash kernel for arch "
