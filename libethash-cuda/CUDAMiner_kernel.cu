...
{
    __shared__ uint32_t c_dag[PHIHASH_CACHE_WORDS];
    ...
    for (uint32_t word = threadIdx.x*PHIHASH_DAG_LOADS; word < PHIHASH_CACHE_WORDS; word += blockDim.x*PHIHASH_DAG_LOADS)
    {
        dag_t load = g_dag[word/PHIHASH_DAG_LOADS];
        for(int i=0; i<PHIHASH_DAG_LOADS; i++)
    ...
    #pragma unroll 1
    for (uint32_t h = 0; h < PHIHASH_LANES; h++)
    {
        uint32_t mix[PHIHASH_REGS];

        hash_seed[0] = __shfl_sync(0xFFFFFFFF, state2[0], h, PHIHASH_LANES);
        hash_seed[1] = __shfl_sync(0xFFFFFFFF, state2[1], h, PHIHASH_LANES);

        // initialize mix for all lanes
        fill_mix(hash_seed, lane_id, mix);

        #pragma unroll 1
        for (uint32_t l = 0; l < PHIHASH_CNT_DAG; l++)
            PhihashLoop(l, mix, g_dag, c_dag, hack_false);
        ...
        for (int i = 0; i < PHIHASH_LANES; i += 8)
            #pragma unroll
            for (int j = 0; j < 8; j++)
                fnv1a(digest_temp.uint32s[j], SHFL(digest_lane, i + j, PHIHASH_LANES));
